...
python get_paper_citation_from_json.py --dblp_json 'dblp_json/OSDI.json' --citation_json 'citation_json/OSDI.json'
...
parser.add_argument('--dblp_json', type=str, help="(input) path of the input dblp json file, e.g., 'dblp_json/OSDI.json'")
parser.add_argument('--citation_json', type=str, help="(output) path of the output citation json file, e.g., 'citation_json/OSDI.json'")
...
citation_json = args.citation_json
...
def get_paper_list_from_dblp_json(dblp_json_path):
  """
  return a list of json (dict) objects, each is a paper
  """

  with open(dblp_json_path, 'r') as f:
...
if __name__ == '__main__':
    ...
    papers = get_paper_list_from_dblp_json(dblp_json)

    citation_json = dict()
    if os.path.exists(citation_json):
